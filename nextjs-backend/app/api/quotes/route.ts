...
export async function GET(req: NextRequest) {
  ...
  return NextResponse.json(quotes.map(withTotals));
}
...
export async function POST(req: NextRequest) {
  ...
  if (!number || (await prisma.quote.findUnique({ where: { number } }))) {
    number = await nextDocumentNumber("quote", "Q");
  }
  ...
  return NextResponse.json(withTotals(quote), { status: 201 });
}
